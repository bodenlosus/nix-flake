{ pkgs, specialArgs, ... }:
let
  start-cosmic-ext = pkgs.writeShellApplication {
...
in
{
  environment.systemPackages = [ start-cosmic-ext ] ++ (with pkgs; [ cosmic-settings-daemon ]);
  services.displayManager.sessionPackages = [
    ((pkgs.writeTextFile {
      ...
      text = ''
        ...
        Exec=${start-cosmic-ext}
          /bin/start-cosmic-ext-niri
      '';
    }).overrideAttrs
      (old: { passthru.providedSessions = [ "COSMIC-on-niri" ]; }))
  ];
}
